...
export function Ticker() {
  ...
  return (
    <div className="bg-[var(--ink)] border-y-[2.5px] border-[var(--border-color)] py-3 overflow-hidden group">
            ...
            <li key={i} className="flex items-center">
              <span 
                className={`font-heading text-lg whitespace-nowrap px-4 tracking-[0.01em] ${
                  i % 3 === 2 ? "text-[var(--yellow)]" : "text-white"
                }`}
              >
                {item}
              </span>
              <span className="text-[var(--red)] text-[12px] leading-none shrink-0">■</span>
            </li>
          ))}
        </ul>
      </div>
